...
public class TbPatientsCPTCalculation extends AbstractPatientCalculation
{
...
//Get concepts
Concept cptstarted = Dictionary
		.getConcept(Dictionary.CPT_STARTED);
...
for (Integer ptId : cohort) {
	...
	if (alive.contains(ptId)) {
		...
		List<Obs> obs = Context.getObsService()
				.getObservationsByPersonAndConcept(Context.getPatientService().getPatient(ptId), cptstarted);
