...
public class TbpatientFailedCatICalculation extends AbstractPatientCalculation
{
...
Concept failurecatI = Dictionary.getConcept(Dictionary.FAILURE_CAT_I);
...
for (Integer ptId : cohort) {
	...
	if (alive.contains(ptId)) {
		...
		for(Obs o : obs ) {
			if(o.getValueCoded()==failurecatI){
				requiredObs.add(o);
				failurecat = true;
			}
		}
